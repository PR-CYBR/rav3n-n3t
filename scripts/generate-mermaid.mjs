...
async function buildCISequence() {
  ...
  for (const f of yamls.slice(0, 4)) {
    ...
    // Extract jobs section and find job names
    const jobNames = [];
    // Find the jobs section: from "jobs:" to the end of file
    const jobsMatch = /^jobs:\s*\n([\s\S]*)$/m.exec(text);
    if (jobsMatch) {
      const jobsSection = jobsMatch[1];
      for (const ln of jobsSection.split(/\r?\n/)) {
    ...
    for (const j of jobNames.slice(0, 6)) {
      ...
      // Extract job block: from job name to next job at same level or end of jobs section
      if (jobsMatch) {
        // Don't use m flag so $ matches only true end of string, not end of line
        const jobMatch = new RegExp(`^  ${j}:\\s*\\n([\\s\\S]*?)(?:\\n  [A-Za-z0-9_-]+:\\s*$|$)`, "").exec(jobsMatch[1]);
        const jobBlock = jobMatch ? jobMatch[1] : "";
        const stepNames = Array.from(jobBlock.matchAll(/^\s{6,}-\s+name:\s*(.+)$/gm)).map(x => x[1].trim()).slice(0, 8);
